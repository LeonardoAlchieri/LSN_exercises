...
//
int RANK = 0; //for the random generator: it specifies the rank when executed using MPI parallelization
int CC = 0;

double Leos_rand(int i)
{
    //
    //  Per rendere
    //
    ...
    srand(RANK*rand()%10);
    if(CC == 0)
        cout << "-- First random for rank " << RANK << ": " << std::rand() << endl;
...
public:
    ...
    void shuffle(int rank)
    {
        RANK = rank;
        random_shuffle(order.begin(), order.end(), Leos_rand);
        return;
    }
