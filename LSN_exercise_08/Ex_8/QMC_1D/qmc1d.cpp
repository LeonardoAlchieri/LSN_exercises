...
double variationalWaveFunction(double v)
{
    ...
    {
        return exp(-0.5*(v-mu)*(v-mu)/(sigma*sigma)) + exp(-0.5*(v+mu)*(v+mu)/(sigma*sigma));
    }
}
...
{
	ofstream out("probability_const.dat");
...
void readInput(int argc, char ** argv)
{
    ...
    if(argc == 1)
    {
        ...
        if(argc == 3)
        {
            TYPE_FUNCTION = atoi(argv[2]);
            cout << endl << "---- TYPE:" << TYPE_FUNCTION << endl << endl;
        }
    }
